...
impl Drop for Stack {
    fn drop(&mut self) {
        ...
        let layout =
            Layout::from_size_align(self.size * word_size, word_size).unwrap();
        unsafe {
...
impl Stack {
    pub fn new(size: usize) -> Stack {
        ...
        let ptr = unsafe {
            let layout =
                Layout::from_size_align_unchecked(word_size * size, word_size);
    ...
    pub fn reserve(&mut self, size: usize) {
        ...
        let new_ptr = unsafe {
            ...
            let new_ptr =
                realloc(self.ptr as *mut u8, layout, word_size * new_size);
            if new_ptr.is_null() {
                handle_alloc_error(layout);
            }
            new_ptr as Ptr
    ...
    pub fn get(&self, idx: usize) -> Val {
        Val(unsafe { *self.ptr.add(idx) })
    }

    pub fn set(&self, idx: usize, val: Val) {
        unsafe {
            *self.ptr.add(idx) = val.0;
        }
    }
}
...
impl Mem {
    pub fn new(
        global_initial_size: usize,
        stack_initial_size: usize,
        minor_pool_size: usize,
        major_pool_size: usize,
    ) -> Mem {
        Mem {
            global: Stack::new(global_initial_size),
            stack: Stack::new(stack_initial_size),
